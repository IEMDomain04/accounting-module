...
const Journal = () => {
  ...
  const fetchCurrencies = () => {
    fetch("http://127.0.0.1:8000/api/currencies/")
      .then((res) => res.json())
      .then((data) => {
        const activeCurrencies = data.filter((c) => c.is_active); // only active currencies
  ...
  const handleSubmit = () => {
    const { journalDate, description, invoiceId, currencyId } = journalForm;
    ...
    // Find the full currency object based on the selected name
    const selectedCurrency = currencies.find(
      (c) => c.currency_name === currencyId
    );

    const payload = {
      journal_id: generateNextJournalId(),
      journal_date: journalDate,
      description,
      total_debit: "0.00",
      total_credit: "0.00",
      invoice_id: invoiceId || null,
      currency_id: selectedCurrency?.currency_id || "", // Send actual ID here
    };
